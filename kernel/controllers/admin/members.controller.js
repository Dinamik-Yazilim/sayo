...
function post(dbModel, sessionDoc, req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let data = req.body || {}
      delete data._id
      if (!data.role) return reject('Rol alanı zorunludur')
      if (!data.username) return reject('Kullanıcı adı zorunludur')
      ...
      if (await dbModel.members.countDocuments({ organization: null, username: data.username }) > 0)
        return reject(`Bu kullanıcı adı zaten mevcut`)
...
function put(dbModel, sessionDoc, req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!doc) return reject(`Üye bulunamadı`)
...
function deleteItem(dbModel, sessionDoc, req) {
  return new Promise(async (resolve, reject) => {
    try {
      if (req.params.param1 == undefined) return restError.param1(req, reject)
      if (sessionDoc.member == req.params.param1) return reject(`Kendi hesabınızı silemezsiniz`)
