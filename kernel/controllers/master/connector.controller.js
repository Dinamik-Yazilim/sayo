...
function connectorTest(dbModel, sessionDoc, req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!clientId) return reject(`İstemci kimliği gerekli`)
      if (!clientPass) return reject(`İstemci şifresi gerekli`)
...
function mssqlTest(dbModel, sessionDoc, req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!clientId) return reject(`İstemci kimliği gerekli`)
      if (!clientPass) return reject(`İstemci şifresi gerekli`)
      if (!mssql) return reject(`MSSQL bağlantı bilgisi gerekli`)
      if (!mssql.options) {
