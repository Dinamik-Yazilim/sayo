...
export function OrderLineDialog({ orderDetails, setOrderDetails, rowIndex, children, ioType }: Props) {
  ...
  const save = (e: any) => {
    try {
      if (!itemId) {
        e.preventDefaut()
        toast({ title: 'Hata', description: 'Ürün seçimi gerekli', variant: 'destructive' })
        return
      }

      if (quantity <= 0) {
        toast({ title: 'Hata', description: 'Miktar sıfırdan büyük olmalı', variant: 'destructive' })
        e.preventDefaut()
        return
      }
      if (rowIndex < 0) {
      ...
      toast({ title: 'Hata', description: (err || 'Hata') as string, variant: 'destructive' })
    }
  }
  return (
    <Sheet>
      ...
      <SheetContent>
        ...
        <div className="flex flex-col gap-4">
          ...
          <TsnInput type={'number'} title="Miktar" defaultValue={quantity} onBlur={e => {
            const q = !isNaN(Number(e.target.value)) ? Number(e.target.value) : 0
            setQuantity(q)
            calcAmount(q, price)
          }} />
          <TsnInput type={'number'} title="Fiyat" defaultValue={price} onBlur={e => {
