...
import { getStokAnaGruplari, searchStokAnaGruplari } from '@/mikroerp'

interface StokAnaGrup {
  san_kod: string
  san_isim: string
  alt_grup_sayisi: number
  stok_sayisi: number
}

export default function load() {
  const [gruplar, setGruplar] = useState<StokAnaGrup[]>([])
  ...
  const load = async () => {
    try {
      ...
      const query = searchTerm
        ? searchStokAnaGruplari(searchTerm)
        : getStokAnaGruplari()
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-[120px] text-right">Alt Grup</TableHead>
                <TableHead className="w-[120px] text-right">Stok</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {gruplar.length === 0 ? (
                ...
                gruplar.map((grup) => (
                  <TableRow key={grup.san_kod}>
                    <TableCell className="font-mono font-medium">
                      {grup.san_kod}
                    </TableCell>
                    <TableCell>{grup.san_isim}</TableCell>
