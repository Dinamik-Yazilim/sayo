  ...
  const changeDb = (dbId: string) => {
    ...
    postItem(`/databases/${dbId}`, token)
      .then(result => {
        ...
        Cookies.set('db', dbId)
        toast({ title: 'Veritabanı değiştirildi', description: `database:${result.db.name}`, variant: 'default', duration: 500 })
        ...
        <div className='flex gap-2'>
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectGroup>
                <Button onClick={load} variant={'outline'} className="flex gap-2" ><RefreshCcwDotIcon /> Veritabanlarını Yenile</Button>
              </SelectGroup>
            </SelectContent>
          </Select>
